...
def result(module, sst):
        ...
        for j, s in enumerate(sst):
            if s == ' ':
                if sst[j - 1] in '/*%^><=' and sst[j + 1] in '/*%^><=':
                    raise Exception('space between operators')
            ...
            if st[i] in STBUK:
                ...
                while st[i] in STBUK or st[i] in STNUMBER:
                    ...
                    if (i < n and (hasattr(math, func) or hasattr(module, func)) and (
                        ...
                        break
                if i >= n:
                    if hasattr(math, func) or hasattr(module, func):
                        cc = '$' + func + '@'
                        yield cc
                    elif func in STLO:
                        logic = ''
                    ...
                    if st[i] == '(' and (hasattr(math, func) or hasattr(module, func) or func in FUNCTION):
                        f = ''
                        # k-number of '()',z-number of ','
                        k = 0
                        z = 0
                        j = i
                        while k != 0 or j < n:
    ...
    return calculation(shunting_yard(parse(sst)))
